...
{
    public EventPointRecoder eventPointRecoder;
    ...
    private Dictionary<int, int> KeysDataMap = new();
        ...
        for (int i = 0; i < spectrumData.Length; i++)
        {
            for (int j = 0; j < Herz_PianoKeys.Length; j++)
            {
                if (Mathf.Abs((i + 1) * interval - Herz_PianoKeys[j]) <= 0.05f)
                {
                    ...
                    KeysDataMap.ContainsKey(j) == false
                    && Mathf.Abs((i + 1) * interval - Herz_PianoKeys[j]) <= 1f
                )
                {
                    KeysDataMap[j] = i;
                }
                    ...
                    KeysDataMap.ContainsKey(j) == false
                    && Mathf.Abs((i + 1) * interval - Herz_PianoKeys[j]) <= 2f
                )
                {
                    KeysDataMap[j] = i;
                }
    ...
    void AnalyzeMusic()
    {
        ...
        foreach (var key in KeysDataMap.Keys)
        {
            //find max
            if (
                spectrumData[KeysDataMap[key]] > maxValue
                && spectrumData[KeysDataMap[key]] > threadshold
            )
            {
        ...
        if (maxValue > 0)
        {
            ...
            // eventPoint.frame = 
            // eventPointRecoder.Recoder(eventPoint);
    ...
    void Start()
    {
        ...
        Invoke(nameof(DebugTestResults), thisAudioSource.clip.length);
    }
